...
class OrderController extends Controller
{
    ...
    public function createOrder(Request $request)
    {
        ...
        $data['shipping_address'] = $shippingAdress;
    ...
    public function getOrderStatusByDriver(Request $request)
    {
        $request->validate([
            'status' => 'required|string|in:pending,processing,completed,cancelled,ready_for_delivery,prepared',
        ]);
        ...
        $user = $request->user();
        $orders = Order::where('driver_id', $user->driver_id)->where('status', $request->status)->get();

        return response()->json([
            'status' => 'success',
            'message' => 'Get all order status by driver',
            'data' => $orders
        ]);
    }
    ...
    public function getOrderReadyForDelivery(Request $request)
    {
        ...
        return response()->json([
            'status' => 'success',
            'message' => 'Get all order ready for delivery',
            'data' => $orders
        ]);
    }

    //Update order status for driver
    public function updateOrderStatusByDriver(Request $request, $id)
    {
        $request->validate([
